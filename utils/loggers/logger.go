...
func (h *Handler) Handle(ctx context.Context, r slog.Record) error {
	...
	if err != nil {
		return fmt.Errorf("error when marshalling attrs: %s", err)
	}
...
func (h *Handler) computeAttrs(ctx context.Context, r slog.Record) (map[string]any, error) {
	...
	if err := h.h.Handle(ctx, r); err != nil {
		return nil, fmt.Errorf("error when calling inner handler's handler : %w", err)
	}

	var attrs map[string]any
	err := json.Unmarshal(h.b.Bytes(), &attrs)
	if err != nil {
		return nil, fmt.Errorf("error when unmarshling inner handler's handler resultt: %w", err)
	}
	return attrs, nil
}
