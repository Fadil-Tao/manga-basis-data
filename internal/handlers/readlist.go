...
type ReadlistRepo interface {
	...
	DeleteReadlistItem(ctx context.Context,  userId int, readlistId int, mangaId int) error
	UpdateReadlistItemStatus(ctx context.Context, readlistId int,mangaId int,status string, userId int) error
...
func (rl *ReadlistHandler) DeleteReadlistItem(w  http.ResponseWriter, r *http.Request){
	readlistId , err := strconv.Atoi(r.PathValue("readlistId"))
	if err != nil {
		JSONError(w, map[string]string{
			"message": "internal server error",
		}, http.StatusInternalServerError)
	...
	if err := rl.Repo.DeleteReadlistItem(r.Context(),userId,readlistId,mangaId ); err != nil {
		if statusCode(err) != http.StatusInternalServerError{
			JSONError(w, map[string]string{
				"message": err.Error(),
			}, statusCode(err))
...
func (rl *ReadlistHandler) UpdateReadlistItemStatus(w http.ResponseWriter, r *http.Request){
	...
	if err := rl.Repo.UpdateReadlistItemStatus(r.Context(),readlistId,mangaId,statusReadlist.Status, userId); err != nil {
		if statusCode(err) != http.StatusInternalServerError{
			JSONError(w, map[string]string{
				"message": err.Error(),
			}, statusCode(err))
